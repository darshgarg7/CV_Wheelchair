...
def evaluate_model(model, X_val: np.ndarray, y_val: np.ndarray):
    """
    Evaluates the model's performance on validation data and logs precision, recall, and F1-score.
...
def train_model():
    ...
    - Load and augment data using GAN.
    ...
    try:
        ...
        assert images_augmented.shape[0] == gestures_augmented.shape[0], \
